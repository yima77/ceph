...
protected:
  ...
  std::tuple<uint32_t, std::string> encrypt(const std::string& secret_id, const std::string& secret) override;

  std::tuple<bool, uint32_t, std::string> decrypt(uint32_t key_id, const std::string& secret_id, const std::string& secret) override;
...
{
  ceph::bufferptr key(encrypt_key.key.data(), encrypt_key.key.length());
  CryptoKey ck{CEPH_CRYPTO_AES, ceph_clock_now(), key};
  ...
  ceph::bufferptr unique_key((const char*)hash.v, hash.SIZE);
  return CryptoKey{CEPH_CRYPTO_AES, ceph_clock_now(), unique_key};
}

std::tuple<bool, uint32_t, std::string> RGWSecretEncrypterImpl::decrypt(uint32_t key_id, const std::string& secret_id, const std::string& secret)
{
  ...
  if (key_found == db_in_use->end()) {
    if (key_id == 0) {
      return std::make_tuple(true, 
                            (enabled && !db_in_use->empty()) ? db_in_use->rbegin()->first : 0,
                             secret);
    } else {
  ...
  RGWEncryptKey &key_to_use = key_found->second;

  auto unique_key = make_unique_key(key_to_use, secret_id);

  std::string key_str;
  unique_key.to_str(key_str);
  ldout(cct, 1) << "INFO: unique key to encrypt/decrypt: " << key_str << " for secret: " << secret << dendl;
  auto* cryptohandler = cct->get_crypto_handler(CEPH_CRYPTO_AES);

  if (cryptohandler->validate_secret(unique_key.get_secret()) < 0) {
    ldout(cct, 1) << "ERROR: Invalid rgw secret encryption key, please ensure its length is 16" << dendl;
    return std::make_tuple(false, 0, secret);
  }

  std::string error;
  std::unique_ptr<CryptoKeyHandler> keyhandler(cryptohandler->get_key_handler(unique_key.get_secret(), error));
  if (! keyhandler) {
  ...
  if (ret < 0) {
    ldout(cct, 1) << "ERROR: fail to decrypt secret: " << error << dendl;
    return std::make_tuple(false, 0, secret);
  }
  return std::make_tuple(true, 
                         enabled ? db_in_use->rbegin()->first : 0, 
                         out.to_str());
}

std::tuple<uint32_t, std::string> RGWSecretEncrypterImpl::encrypt(const std::string& secret_id, const std::string& secret)
{
  ...
  auto& key_to_use = db_in_use->rbegin()->second;
  ...
  return std::make_tuple(key_to_use.id, out_encoded.to_str());
}

} // namespace secret_encrypt
} // namespace rgw
