...
public:
  ...
  // Encrypt a secret. Return a pair of:
  ...
  // 2, Encrypted secret.
  virtual std::tuple<uint32_t, std::string> encrypt(const std::string& secret_id, const std::string& secret) = 0;

  // Decrypt a secret. Return a pair of:
  ...
  // 3, Decrypted secret.
  virtual std::tuple<bool, uint32_t, std::string> decrypt(uint32_t key_id, const std::string& secret_id, const std::string& secret) = 0;
};
