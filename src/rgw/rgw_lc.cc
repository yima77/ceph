...
int RGWLC::handle_lc_mp_ops(RGWLC::LCWorker *worker,
                            rgw::sal::Driver *driver,
                            rgw::sal::Bucket* bucket,
                            ...
                            boost::asio::yield_context yield)
{
  ...
  ldpp_dout(this, 20) << __func__ << "(): initiate obj lister" << dendl;
  int ret = ol.init(this, yield);
  if (ret < 0) {
    ldpp_dout(this, 0) << "ERROR: driver->list_objects(): ret=" << ret << dendl;
    return ret;
  }
  ...
  for (auto offset = 0; ol.get_obj(this, yield, &o /* , fetch_barrier */); ++offset, ol.next()) {
    ...
    for (auto* op : ops) {
      if (obj_has_expired(this, cct, obj.meta.mtime, op->mp_expiration)) {
        rgw_obj_key key(obj.key);
        auto mpu = bucket->get_multipart_upload(key.name);
        auto sal_obj = bucket->get_object(key);

        string etag;
        ret = sal_obj->load_obj_state(this, yield, true);
        if (ret < 0) {
          return ret;
        }
        bufferlist bl;
        ...
        ret = mpu->abort(this, cct, yield);
        if (ret == 0) {
          const auto event_type = rgw::notify::ObjectExpirationAbortMPU;
          send_notification(this, yield, driver, sal_obj.get(), bucket, etag, size,
    ...
    if ((offset % 100) == 0) {
      if (worker_should_stop(stop_at, once)) {
        ldpp_dout(this, 0) << __func__ << " interval budget EXPIRED worker="
          << worker->ix << " bucket=" << bucket->get_name()
          << dendl;
        return 0;
      }
    }
...
int RGWLC::handle_lc_ops(RGWLC::LCWorker *worker,
                         rgw::sal::Driver *driver,
                         rgw::sal::Bucket* bucket,
                         const std::string& prefix,
                         const std::string& shard_id,
                         ...
                         boost::asio::yield_context yield)
{
  ...
  for (auto* op : ops) {
    op_env oenv(*op, driver, worker, bucket, ol);
  ...
  auto flush_key_group = [&] {
    ...
    if (!bucket->versioned() || key_group.size() == 1) {
      ...
      for (auto& entry : key_group) {
        ...
        if (std::any_of(entry.rules.begin(), entry.rules.end(),
                [](const LCOpRule& r) { return r.needs_tags(); }))
          cached_tags_ptr = fetch_obj_tags(this, bucket, entry.obj,
                                           cached_tags, yield);

        for (auto& rule : entry.rules) {
          if (rule.needs_tags() &&
              !entry.obj.is_delete_marker() &&
              cached_tags_ptr &&
          ...
          ldpp_dout(this, 20)
            << __func__ << "(): key=" << entry.obj.key << dendl;
          int ret = rule.process(entry.obj, this, batch_counters, yield, cached_tags_ptr);
          if (ret < 0) {
            ldpp_dout(this, 20)
      ...
      std::vector<tag_entry> tags(key_group.size());
      std::vector<action_ref> dels, non_dels;

      for (size_t ei = 0; ei < key_group.size(); ++ei) {
        auto& obj = key_group[ei].obj;
        if (std::any_of(key_group[ei].rules.begin(),
                key_group[ei].rules.end(),
                [](const LCOpRule& r) { return r.needs_tags(); }))
          tags[ei].ptr = fetch_obj_tags(this, bucket, obj,
                                        tags[ei].buf, yield);
        bool has_del = false;
        std::vector<action_ref> entry_non_dels;
        entry_non_dels.reserve(key_group[ei].rules.size());
        for (size_t ri = 0; ri < key_group[ei].rules.size(); ++ri) {
          auto& rule = key_group[ei].rules[ri];
          if (rule.needs_tags() &&
          ...
          auto* act = rule.evaluate(
              key_group[ei].obj, this, tags[ei].ptr, yield);
          if (!act) continue;
          if (act->is_delete()) {
            /*
      ...
      for (auto& r : non_dels)
        key_group[r.ei].rules[r.ri].execute(
            *r.a, key_group[r.ei].obj, this,
      ...
      for (size_t i = 0; i < dels.size(); ++i)
        items.push_back({key_group[dels[i].ei].obj.key, i});
      ...
      for (size_t i = 0; i < key_group.size(); ++i)
        batch_counters->decrement_pending();
    }
    key_group.clear();
  };
  ...
  for (auto offset = 0; ol.get_obj(this, yield, &o /* , fetch_barrier */); ++offset, ol.next()) {
    ...
    ldpp_dout(this, 20) << __func__ << "(): update rules=" << o->key.name << dendl;
			     ...
			     boost::asio::yield_context yield)
{
  ...
  for (auto prefix_iter = grouped_ops.begin(); prefix_iter != grouped_ops.end();
    ...
    for (int index_sid = (num_shards == RGW_NO_SHARD ? RGW_NO_SHARD : 0); index_sid < num_shards; ++index_sid) {
      ldpp_dout(this, 20) << __func__ << "(): process shard=" << index_sid << " by worker ix: " << worker->ix << dendl;
      workpool.spawn([this, prefix_iter_copy=prefix_iter, bucket=bucket.get(),
                    shard_id, worker, once, stop_at, index_sid,
                    &active_ops, &active_mp_ops, &batch_counters]
                   (boost::asio::yield_context yield) mutable {

        // Regular object handling
        handle_lc_ops(worker, driver, bucket, prefix_iter_copy->first, shard_id,
                                 index_sid, active_ops, stop_at, once, &batch_counters, yield);

        // Multipart object handling
        handle_lc_mp_ops(worker, driver, bucket, prefix_iter_copy->first, shard_id,
                                    index_sid, active_mp_ops, stop_at, once, &batch_counters, yield);
      }); // workpool.spawn
    }
  }
